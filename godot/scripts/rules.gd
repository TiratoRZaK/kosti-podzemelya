...
## Форы за первый ход больше нет: право хода разыгрывается битвой, и выигравший
## её не должен получать сверху ещё и очки — на экране это выглядело как «кому-то
## просто дали десять очков». Раньше компенсация лечила перекос в пользу
## отвечающего; теперь перекос лечит сама битва — кто выиграл бросок, тот и решил.
const FIRST_MOVE_KOMI := 0
